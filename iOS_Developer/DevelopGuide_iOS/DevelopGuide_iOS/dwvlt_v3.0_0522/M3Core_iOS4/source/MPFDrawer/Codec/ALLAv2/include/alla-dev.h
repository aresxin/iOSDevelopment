...
private:
    ...
    int		feed_marker		(const marking marker,
					 const int i);
    void	decode_line_internal	(void);
};
/*---------------------------------------------------------------------------*/
// Info
struct BufferInfo {
...
struct Sys {
    MY_CALLBACK		callback_in;
    void * 		userdata_in;
    MY_CALLBACK		callback_out;
    void * 		userdata_out;
    int			component;
...
public:
    ...
    Info 				(void);
    ~Info 				(void) {};
    void	set_callbacks		(MY_CALLBACK	in_callback,
					 void *		in_userdata,
    					 MY_CALLBACK	out_callback,
					 void *		out_userdata);
    ...
    void	set_differer		(Status *	status,
					 const int	order);

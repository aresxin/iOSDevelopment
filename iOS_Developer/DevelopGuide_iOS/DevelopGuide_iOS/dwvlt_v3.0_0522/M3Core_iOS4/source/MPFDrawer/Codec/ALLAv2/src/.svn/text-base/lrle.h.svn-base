...
public:
    LrleDec					(Status *	status,
						 const Info & info);
    ~LrleDec					(void);
    void		init			(Status *	status);
    void		apply_outfield		(void);
    void		enter_line		(Status *	status);
    void 		exit_line		(Status *	status,
						 const int next_line_phase);
    void		copy_line		(void)
			    { imagebuffer->copy_line (); }
    int			feed_last_run		(Status *	status);
    int			feed_symbol		(Status *	status,
						 const int offset,
						 const Symbol & symbol);
private:
    int			feed_run		(Status *	status,
						 const int offset_byte);
...
Lrle (Status *		status,
      const Info &	info_)
    : imagebuffer (new B (status, info_)), info (info_)
{
...
template<class B, class R>
INLINE void Lrle<B, R>::
init_referer (Status *		status)
{
    for (int i = 0; i < prediction_num; i++) {
	referer[i]->init_pref (status, imagebuffer.get (),
				info.sys.coding_component);
	if (IS_BAD (*status)) return;
    }
}
...
template<class B, class R>
void Lrle<B, R>::
apply_initial_order (Status *	status)
{
	...
	if (pred == A)
		...
		THROW (status, "Lrle::apply_initial_order(): RUC defined doubly", 190);
		return;
	    }
	    rp = i;
...
feed_last_run (Status *		status)
{
	...
	order[0]->distribute_run (status, 0, trun * info.sys.coding_component);
...
#ifndef VC6
template<class T> const Symbol & LrleEnc::
look_last_run (Type2Type<T>)
{
    LOOK_LAST_RUN(T)
}

template<class T> void LrleEnc::
remove_sentinel (Type2Type<T>)
{
    REMOVE_SENTINEL(T)
}
...
#ifndef VC6
template<class T> INLINE const Symbol & LrleEnc::
look_longest_run (const int offset,
		  Type2Type<T>,
		  Int2Type<5>)
{
	  ...
	  const int	offset)
{
	...
	distribute_run (status, offset * info.sys.coding_component,
	     ...
	     const Symbol &	symbol)
{
	...
	const int run (feed_run (status, offset));

...
public:
    ...
    ~Distributor				(void) {};
    void		distribute_run		(Status *	status,
...
#ifndef VC6
template<class T> INLINE T Predictor::
...
#ifndef VC6
template<class T> INLINE int Predictor::
look_run (const int offset,
	  Type2Type<T>)
{
    LOOK_RUN(T)
...
#ifndef VC6
template<class T> INLINE void Predictor::
set_ructab (const int offset,
	    Type2Type<T>)
{
    SET_RUCTAB(T)
		...
		const int	run_byte)
{
	...
	case ruc:
	    ...
	    break;
	case noinuse:
	    THROW (status, "Distributor::distribute_run(): stated is no in use", 200);
	    break;
    }
}

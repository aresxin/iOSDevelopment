 ...
 * 
 * File: IMG_RopSequence2Step.h - 2ステップROP描画シーケンスクラスの定義
 * Yozo Kashima 18.Nov.2001
 *
 */

#ifndef INC_IMG_RopSequence2Step_h
#define INC_IMG_RopSequence2Step_h
...
/*
 * IMG_RopSequence2Stepクラスの定義
 */

typedef struct img_ropsequence2step {
	IMG_RopSequence		super;		/* SuperClass */
} IMG_RopSequence2Step;

/*
 * ファクトリー関数
 */
extern int IMG_RopSequence2Step_CreateObjects(
...
extern int IMG_RopSequence2Step_Reconstruct(
	IMG_RopSequencePtr		self);

extern IMG_BOOL IMG_RopSequence2Step_IsValid(
	IMG_RopSequencePtr			self);

extern IMG_BOOL IMG_RopSequence2Step_IsSupported(
	IMG_RopMode					ropMode,	/* ROPシーケンスの最初のステップのROPモード(+) */
	IMG_UINT32					ropCode);	/* ROPシーケンスの最初のステップのROPコード(+) */

/* INC_IMG_RopSequence2Step_h */
#endif

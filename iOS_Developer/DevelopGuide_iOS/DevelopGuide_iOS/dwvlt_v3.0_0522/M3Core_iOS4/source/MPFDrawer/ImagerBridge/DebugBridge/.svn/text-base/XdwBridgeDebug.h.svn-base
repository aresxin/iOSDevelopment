/*!
  @file  XdwBridgeDebug.h
  @brief XdwBridgeDebugクラスの定義を記述したIncludeファイル
 ...
 */
#ifndef _XDW_BRIDGE_DEBUG_H_
#define _XDW_BRIDGE_DEBUG_H_
 ...
 */
class XdwBridgeDebug : public XdwBridgeDrawIF
{
public:
    /*! コンストラクタ
     ...
     */
    virtual ~XdwBridgeDebug();
    
    /**
     * @brief インスタンスを初期化する
     *
     * @param log_file 各メソッドのパラメータをLogファイルに記述するインターフェースのインスタンス
     *
     * @recutn XdwErrorCode参照
     */
    virtual XdwErrorCode Initialize(XdwLogInterface* log_file);
     ...
     * @retval XDW_ERRORCODE_SUCCESS 正常終了
     */
    virtual XdwErrorCode SetRectangleToClip( const XdwRectangleVector &rect,
                                             XdwGSClipLogic mode );
...
protected:
    ...
    /**
     * @brief Unicode -> MBCS変換
     *
     * @param string Unicodeが格納されているバッファ
     * @param size stringの配列サイズ
     *
     * @return MBCS変換された文字列
     */
    char* UnicodeToMBCS(unsigned char* string, long size);
};

...
@implementation FreeGifLoader

static FreeGifLoader * instance;

@synthesize imageView;
@synthesize busyDecoding;

+ (FreeGifLoader *) sharedInstance
{
    if (instance == nil)
    {
        instance = [[FreeGifLoader alloc] init];
    }
...
{   
    ...
    [[FreeGifLoader sharedInstance] addToQueue: agqo];
    [agqo release];
    
    if ([[FreeGifLoader sharedInstance] busyDecoding] != YES)
    {
        [[FreeGifLoader sharedInstance] setBusyDecoding: YES];
        
        // Asynchronous loading for URL's, else the GUI won't appear until image is loaded.
        [[FreeGifLoader sharedInstance] performSelector:@selector(asynchronousLoading) withObject:nil afterDelay:0.0];
    }
